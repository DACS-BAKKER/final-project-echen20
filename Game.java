...
public class Game {
    ...
    City[] cities;
    Stack<InfectionCard> infectionDeck;
    Stack<PlayerCard> playerDeck;
    Stack<InfectionCard> infectionDiscard;
    LinkedList<City> researchStations; // list of all cities with research stations
    LinkedList<Player> playersOnCurrCity;
    City lastEpidemicLocation; // city where last epidemic card was pulled for

    int difficulty; // 0 = easy, 1 = medium, 2 = hard, decides how many epidemic cards are put in the deck
    boolean[] cured; // each index - 0: blue, 1: yellow, 2: black, 3: red
    boolean[] eradicated; // each index - 0: blue, 1: yellow, 2: black, 3: red
    int[] totalCubes; // each index - 0: blue, 1: yellow, 2: black, 3: red
    int infectionRate; // goes up each time an epidemic card is pulled, index for infectionRatePossibilities
    int[] infectionRatePossibilities = {2, 2, 2, 3, 3, 4, 5}; // how many cards are pulled each turn (depending on number
                                                              // of epidemic cards pulled
    int outbreakCounter; // how many outbreaks have occurred

    LinkedList<City> outbrokenCities; // cities since last outbreak that have had outbreaks
